...
export default function* execute(action: any): any {
  try {
    ...
    yield put({ type: Types.UPDATE_VITAL_SIGNS_REQUEST, refresh })

    const repository: VitalSignsRepository = new VitalSignsDataRepository()

    const response = yield call(
      repository.updateVitalSigns,
      codOrder,
      codAnalysis,
      temperature,
      pulse,
    ...
    yield put({
      type: Types.UPDATE_VITAL_SIGNS_SUCCESS,
      data: response[0],
    })
  } catch (error) {
    yield put({
      type: Types.UPDATE_VITAL_SIGNS_FAILURE,
      error: true,
    })
  }
}
